...
function ajouterProduit($image, $nom, $description, $libelle, $prix) {
    $conn = connectDB();
    if ($conn) {
        $sql = "INSERT INTO tableintrants (image, nom, description, libelle, prix) VALUES (:image, :nom, :description, :libelle, :prix)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':image' => $image,
            ':nom' => $nom,
...
function supprimerProduit($id) {
    $conn = connectDB();
    if ($conn) {
        $sql = "DELETE FROM tableintrants WHERE id = :id";
        $stmt = $conn->prepare($sql);
...
function modifierProduit($id, $image, $nom, $description, $libelle, $prix) {
    $conn = connectDB();
    if ($conn) {
        $sql = "UPDATE tableintrants SET image = :image, nom = :nom, description = :description, libelle = :libelle, prix = :prix WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':image' => $image,
            ':nom' => $nom,
...
<head>
    ...
    <title>Administration des Intrants</title>
    <style>
        body {
...
<body>
<div class="container">
    <h2>Administration des Intrants</h2>

    <!-- Formulaire pour ajouter un produit -->
    <h3>Ajouter des Intrants</h3>
    <form action="" method="post" enctype="multipart/form-data">
        <div class="form-group">
            <label for="image">Image :</label>
            <input type="file" style = "width="300px" name="image" required>
        </div>
        <div class="form-group">
        ...
        <button type="submit" name="ajouter">Ajouter le service</button>
