...
function supprimerProduit($id) {
    $conn = connectDB();
    if ($conn) {
        $sql = "DELETE FROM tableservice WHERE id = :id";
        $stmt = $conn->prepare($sql);
...
function modifierProduit($id, $image, $nom, $description, $libelle, $prix) {
    $conn = connectDB();
    if ($conn) {
        $sql = "UPDATE tableservice SET image = :image, nom = :nom, description = :description, libelle = :libelle, prix = :prix WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':image' => $image,
            ':nom' => $nom,
...
<div class="container">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <button type="submit" name="ajouter">Ajouter le service</button>"			"<a href="logout.php">Se déconnecter</a>
    </form>
